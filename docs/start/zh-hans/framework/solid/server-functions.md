...
translation-updated-at: '2025-05-06T22:06:25.770Z'
id: server-functions
title: 服务端函数
---
...
- 它们没有固定的公开 URL（但很快将支持此功能！）
- 可以从应用程序的任何位置调用，包括加载器 (loader)、钩子 (hook)、组件等，但不能从 API 路由中调用。

然而，它们与常规 API 路由的相似之处在于：

- 可以访问请求上下文，允许读取标头、设置 Cookie 等
- 可以访问敏感信息（如环境变量），而不会暴露给客户端
- 可用于执行任何类型的服务端逻辑，例如从数据库获取数据、发送邮件或与其他服务交互
- 可以返回任何类型的值，包括原始值、可 JSON 序列化的对象，甚至是原始的 Response 对象
- 可以抛出错误，包括重定向 (redirect) 和未找到 (notFound) 错误，这些错误可由路由器自动处理

> 服务端函数与 "Solid 服务端函数 (Solid Server Functions)" 有何不同？
>
> - TanStack 服务端函数不绑定特定前端框架，可与任何前端框架或无框架环境配合使用。
> - TanStack 服务端函数基于标准 HTTP 请求，可频繁调用而不会遭遇串行执行的瓶颈。

## 工作原理

服务端函数可在应用程序的任何位置定义，但必须在文件的顶层定义。它们可在应用程序各处调用，包括加载器、钩子等。传统上，这种模式称为远程过程调用 (RPC)，但由于这些函数的同构特性，我们将其称为服务端函数。

- 在服务端打包时，服务端函数的逻辑保持不变。由于它们已位于正确位置，无需额外处理。
- 在客户端，服务端函数会被移除；它们仅存在于服务端。客户端对服务端函数的任何调用都将替换为向服务器发送 `fetch` 请求以执行该函数，并将响应返回给客户端。
...
> 我们要感谢 [tRPC](https://trpc.io/) 团队为 TanStack Start 的服务端函数设计提供的灵感以及在实现过程中的指导。我们非常喜爱（并推荐）将 tRPC 用于 API 路由，因此我们坚持服务端函数也应获得同样的一流待遇和开发者体验。感谢！

服务端函数通过 `@tanstack/solid-start` 包中的 `createServerFn` 函数定义。该函数接受一个可选的 `options` 参数用于配置 HTTP 方法和响应类型等，并允许您链式定义函数体、输入验证、中间件等。以下是一个简单示例：
...
- `'raw'`：直接返回原始 Response 对象，支持流式响应和自定义标头
...
> [!WARNING]
> 服务端函数不能从 API 路由中调用。如果需要在服务端函数和 API 路由之间共享业务逻辑，请将共享逻辑提取为可被两者导入的实用函数。
...
服务端函数可配置为在运行时验证输入数据，同时增加类型安全性。这有助于确保在执行服务端函数前输入类型正确，并提供更友好的错误消息。
...
虽然我们强烈建议使用验证库来验证网络 I/O 数据，但您可能出于某些原因不想验证数据，但仍希望保持类型安全。为此，可以使用标识函数作为 `validator` 向服务端函数提供类型信息，将输入和/或输出类型化为正确的类型：
...
服务端函数可以接受可 JSON 序列化的对象作为参数。这对于向服务器传递复杂数据结构非常有用：
...
除了服务端函数接受的单个参数外，您还可以使用 `@tanstack/solid-start/server` 中的实用程序从任何服务端函数中访问服务器请求上下文。在底层，我们使用 [Unjs](https://unjs.io/) 的 `h3` 包执行跨平台 HTTP 请求。
...
有关可用上下文函数的完整列表，请参阅所有可用的 [h3 方法](https://h3.unjs.io/utils/request) 或查看 [@tanstack/start-server-core 源代码](https://github.com/TanStack/router/tree/main/packages/start-server-core/src)。
...
使用 `getHeaders` 函数从服务端函数内部访问所有标头：
...
要从服务端函数返回任何原始值或可 JSON 序列化的对象，只需从函数中返回值：
...
默认情况下，服务端函数假定任何非 Response 对象返回的是原始值或可 JSON 序列化的对象。

## 响应自定义标头

要响应自定义标头，可以使用 `setHeader` 函数：
...
要响应自定义状态码，可以使用 `setResponseStatus` 函数：
...
要返回原始 Response 对象，可从服务端函数返回一个 Response 对象并设置 `response: 'raw'`：
